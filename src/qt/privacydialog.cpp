...
#include "zcoriumcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMLDN ought to be enough for anybody." - Bill Gates, 2017
    ui->zMLDNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMLDNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMLDN->setEnabled(false);
        ui->pushButtonMintzMLDN->setToolTip(tr("zMLDN is currently disabled due to maintenance."));

        ui->pushButtonSpendzMLDN->setEnabled(false);
        ui->pushButtonSpendzMLDN->setToolTip(tr("zMLDN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMLDNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMLDN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMLDN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMLDN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMLDN in ") +
...
void PrivacyDialog::on_pushButtonSpendzMLDN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMLDN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMLDN
        sendzMLDN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMLDN
    sendzMLDN();
}

void PrivacyDialog::on_pushButtonZCoriumControl_clicked()
{
    ZCoriumControlDialog* zMLDNControl = new ZCoriumControlDialog(this);
    zMLDNControl->setModel(walletModel);
    zMLDNControl->exec();
}

void PrivacyDialog::setZCoriumControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMLDNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMLDN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Corium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMLDNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMLDN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CORIUM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMLDN</b>";
    ...
    // use mints from zMLDN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCoriumControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCoriumControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcorium selector in case it was used
    ZCoriumControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMLDN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Corium, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMLDN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMLDN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMLDN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMLDN "));
    ui->labelzMLDNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMLDNSyncStatus->setVisible(fShow);
}
