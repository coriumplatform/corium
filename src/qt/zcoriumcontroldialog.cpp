...
#include "ui_zcoriumcontroldialog.h"
...
std::list<std::string> ZCoriumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCoriumControlDialog::listMints;

ZCoriumControlDialog::ZCoriumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCoriumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCoriumControlDialog::~ZCoriumControlDialog()
{
    delete ui;
}

void ZCoriumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCoriumControlDialog::updateList()
{
...
void ZCoriumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCoriumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCorium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCoriumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCoriumControlDialog::GetSelectedMints()
{
...
void ZCoriumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
